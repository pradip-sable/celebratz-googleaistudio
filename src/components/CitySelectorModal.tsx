...
export const CitySelectorModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs animate-fadeIn">
      <div 
        className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-stone-200 overflow-hidden flex flex-col max-h-[90vh] animate-scaleUp"
      ...
      >
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] text-stone-500">Verified contacts & calendars</span>
            </div>
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-serif font-bold text-base text-stone-900">{city.name}</span>
                        <span className="text-xs text-stone-500">&bull; {city.state}</span>
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                          {city.badge}
                        </span>
                      </div>
                      <p className="text-xs text-stone-600 leading-snug">{city.tagline}</p>
                      <div className="flex flex-wrap items-center gap-1.5 pt-1">
                        <span className="text-[10px] font-semibold text-stone-600">Top Hubs:</span>
                        {city.popularHubs.slice(0, 5).map(hub => (
                          <span key={hub} className="px-1.5 py-0.5 rounded bg-stone-100 text-stone-600 text-[10px]">
                            {hub}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="shrink-0 flex items-center gap-2">
                      {isSelected ? (
                        <div className="px-3 py-1.5 rounded-xl bg-emerald-800 text-white text-xs font-bold flex items-center gap-1.5 shadow-xs">
                          <CheckCircle2 className="w-3.5 h-3.5" />
                          <span>Active Market</span>
                        </div>
                      ) : (
                        <div className="px-3 py-1.5 rounded-xl bg-stone-100 hover:bg-emerald-100 text-stone-800 hover:text-emerald-900 text-xs font-bold transition-colors">
                          Switch to Pune
                        </div>
                      )}
                    </div>
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between gap-1">
                        <span className="font-serif font-bold text-sm text-stone-900">{city.name}</span>
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-stone-100 text-stone-600 border border-stone-200">
                          {city.badge}
                        </span>
                      </div>
                      <div className="text-[11px] text-stone-500">{city.state}</div>
                      <p className="text-[11px] text-stone-600 mt-1 leading-snug line-clamp-2">{city.tagline}</p>
                      
                      <div className="mt-2 flex flex-wrap gap-1">
                        {city.popularHubs.slice(0, 3).map(h => (
                          <span key={h} className="text-[9px] px-1.5 py-0.5 rounded bg-stone-50 border border-stone-200 text-stone-500">
                            {h}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="pt-2 border-t border-stone-100 flex items-center justify-between gap-2">
                      ...
                      <button
                        onClick={() => handleSelectCity(city)}
                        className="text-[10px] px-2 py-1 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold"
                      >
                        Preview Mode
                      </button>
                    </div>
        ...
        <div className="px-6 py-3.5 bg-stone-50 border-t border-stone-200 flex items-center justify-between text-xs text-stone-500">
          <div className="flex items-center gap-1.5">
            ...
            <span>Active City: <strong className="text-stone-900 font-bold">{activeCity?.name || 'Pune'}</strong></span>
          </div>
          <button
            onClick={() => setIsCitySelectorOpen(false)}
